...
class login extends connection
{
	...
	{	
		...
		$query = "SELECT * FROM  tbl_user WHERE `userName`='$username' AND `userPassword`='$password'";
		$queryValue = $this->aSinRow($query);
		if (!empty($queryValue))
		{
			$_SESSION['userId']				= $queryValue['userId'];
			$_SESSION['ins_id']				= $queryValue['userId'];
			$_SESSION['userEmail']			= $queryValue['email'];
			$_SESSION['name']	 			= $queryValue['name'];
			$_SESSION['departmentName']		= $queryValue['departmentName'];
			$_SESSION['departmentSubName']	= $queryValue['departmentSubName'];
			$_SESSION['departmentID']		= $queryValue['departmentID'];
			return 1;
		}else
		{
